...
def main(_):
    os.makedirs('generate/tweet', exist_ok=True)
    with tf.Graph().as_default(), tf.Session(config=config_tf) as session:
        ...
        start_words = []
        data = open('data/tweet.txt', 'r').readlines()
        ...
        outfile = open('./generate/tweet/tweet_' + str(bit_num) + 'bit' + '_' + index + '.txt', 'w')
        bitfile = open('./generate/tweet/tweet_' + str(bit_num) + 'bit' + '_' + index + '.bit', 'w')
        ...
        while count < 100:
            ...
            for i in range(len_of_generation - 2):
                ...
                for b in range(2**int(bit_num)):
                    if bit_stream[bit_index:bit_index+b+1] in codes:
                        code_index = codes.index(bit_stream[bit_index:bit_index+b+1])
                        gen = words_prob[code_index][0]
                        test_data = np.int32(gen)
                        gen_res.append(idx_to_word[gen])
                        if idx_to_word[gen] in ['\n', '']:
                            break
                        bit += bit_stream[bit_index: bit_index+b+1]
