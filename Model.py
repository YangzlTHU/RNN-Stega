...
class Model(object):
    def __init__(self, is_training, config):
        ...
        for i in range(config.num_layers):
            stack_rnn.append(tf.contrib.rnn.BasicLSTMCell(size, forget_bias=0.0, state_is_tuple=False))
        cell = tf.contrib.rnn.MultiRNNCell(stack_rnn, state_is_tuple=False)
        self._initial_state = cell.zero_state(batch_size, tf.float32)
        '''
